...
import com.jxshi.clu.algorithms.DBSCAN;
...
public class DBSCANTest {
	...
	public static void main(String[] args) {
		// read data
		String filePath = "D:\\Users\\Jinxin Shi\\eclipse-workspace\\Clustering\\data\\2d-noise\\cluto-t7-10k.data";
		...
		double eps = 15;
		int minPts = 8;
		DBSCAN dbscanModel = new DBSCAN(dataset, eps, minPts);
		dbscanModel.clustering();
		
		// plot results
		plotResults(dbscanModel);

	}
	
	private static void plotResults(DBSCAN dbscanModel) {
		DefaultXYDataset xydataset = new DefaultXYDataset();
		Set<Cluster> clusters = dbscanModel.getClusterSet();
		List<Point> outliers = dbscanModel.getOutliers();
		
		// get coordinates and cluster labels
		for (Cluster clu : clusters) {
			List<Point> members = clu.getMembers();
		...
		for (int i = 0; i < outliers.size(); i++) {
			outlierCoords[0][i] = outliers.get(i).getCoords()[0]; // x coordinate
			outlierCoords[1][i] = outliers.get(i).getCoords()[1]; // y coordinate
		}
		xydataset.addSeries("outlier", outlierCoords);
		
		// plot settings
		String title = "dbscan-clustering ( Eps = " + dbscanModel.getEps() + ", MinPts = " + dbscanModel.getMinPts() + " )";
